...
public:
	...
	*/
	...
	/** 
	Assigns the SPI pins used by the DW1000 before calling begin(). This is required on the
	ESP32 (and other boards with arbitrary SPI pin mapping) when the DW1000 is not connected
	to the default SPI pins. Call this before begin(). On boards with fixed SPI pins (e.g. AVR)
	this call has no effect.

	@param[in] sck  The SPI clock pin (SPICLK).
	@param[in] miso The SPI master-in/slave-out pin (SPIMISO).
	@param[in] mosi The SPI master-out/slave-in pin (SPIMOSI).
	@param[in] ss   The chip select pin (SPICSn).
	*/
	static void setSpiPins(uint8_t sck, uint8_t miso, uint8_t mosi, uint8_t ss);
...
//private:
	...
	/* optional custom SPI pins (used on boards with arbitrary SPI mapping, e.g. ESP32). */
	static uint8_t _sck;
	static uint8_t _miso;
	static uint8_t _mosi;
	static bool _customSpiPins;
