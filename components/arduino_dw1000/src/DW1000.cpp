...
bool    DW1000Class::_customSpiPins = false;
...
void DW1000Class::setSpiPins(uint8_t sck, uint8_t miso, uint8_t mosi, uint8_t ss) {
	_sck  = sck;
	_miso = miso;
	_mosi = mosi;
	_ss   = ss;
...
void DW1000Class::begin(uint8_t irq, uint8_t rst) {
	...
	// start SPI
#if defined(ESP32)
	// ESP32 allows arbitrary SPI pin assignments; use the pins given via setSpiPins()
	// if provided, otherwise fall back to the board defaults.
	if(_customSpiPins) {
	...
	SPI.usingInterrupt(digitalPinToInterrupt(irq)); // not every board supports this, e.g. ESP8266, ESP32
#endif
