...
void sx127x_lora_handle_interrupt(sx127x *device) {
  ...
  if ((value & SX127X_IRQ_FLAG_PAYLOAD_CRC_ERROR) != 0) {
    ...
    /* DIAGNOSTIC (temporary): a packet was received but failed CRC. On the
     * sender this reveals whether ACKs arrive corrupted instead of not at all. */
    ESP_LOGW("sx127x", "DIAG RX CRC error (IRQ=0x%02X)", value);
    return;
  }
